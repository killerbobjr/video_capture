...
namespace ca
{
	...
	typedef void(*frame_callback)(PixelBuffer & buffer, void * );
	...
	class Capability
	{
		/* Capability represents a possibility for a capture device. It often is related to a width/height/fps. */
	public:
		Capability();
	...
	class Device
	{
		/* A device represents a capture device for the implementation. */
	public:
		Device();
		~Device();
		void clear();								/* Resets all members to defaults. */
	...
	class Format
	{
		/* A format is used to describe an output format into which the raw buffers from the webcam can be converted. This is often a feature from the OS, like the AVCaptureVideoDataOutput. */
	public:
	...
	class Settings
	{
		/* A settings object is used to open a capture device. It describes what capability, device and output format (if any) you want to use. */
	public:
	...
	class Frame
	{
		/* A frame object can be used to store information about the frame data for a specific pixel format. */
	public:
		Frame();
		~Frame();
		void clear();
