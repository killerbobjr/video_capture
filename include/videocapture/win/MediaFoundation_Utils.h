...
namespace ca
{
	...
	T string_cast(const wchar_t * src, unsigned int codePage = CP_ACP)
	{
		...
		if (source_length > 0)
		{
			...
			std::vector < char > buffer(length);::WideCharToMultiByte(codePage, 0, src, (int) source_length, & buffer[0], length, NULL, NULL);
...
#endif	// VIDEO_CAPTURE_MEDIA_FOUNDATION_UTILS_H
